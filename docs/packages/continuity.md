...
- **It crosses languages** — a snapshot written here verifies to the same content hash in `SourceRpc.Continuity`, and a journal written here chains to the same hashes and yields the same replay plan there. Checked against fixtures both suites read verbatim rather than asserted; a chain is a stronger claim than a digest, because it is over every document and the order they are in.
