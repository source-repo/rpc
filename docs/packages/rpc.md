...
- **A worker can be a peer, not only a host** — `MessagePortTransport` carries the same flat frame over a `MessagePort`: posted rather than encoded, so nothing parses and the boundary is still real. Symmetric, because a channel has two ends and no broker; presence is an announcement each way and a `peerGone` when the port closes; `carrying` lets the host advertise the rest of the network so the worker can call out to it.
