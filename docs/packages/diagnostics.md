...
- **An exact breakpoint stops on a line** — the same supervisor, handed the gate of a component hosted on its own thread instead of a barrier. It parks the logic between two statements of a handler while the supervisor keeps answering, and a resume continues the same stack rather than re-running it.
- **The kind of pause is a property of the mechanism** — a supervisor takes exactly one, so it cannot claim an exact stop while producing a safe-boundary one, and `exactPause` is advertised from what it was given. A request that never parks is withdrawn, because a component parking later with nobody watching is worse than one that never stopped. `stepping` is still `false`: it is this gate with a frame-depth predicate, which nothing evaluates yet.
