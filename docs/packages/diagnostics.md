...
- **Capabilities are derived from what the host wired** — `diagnosticVariants` when a deployment has a store, fences and a coordinator; the probe flags when there is somewhere for probes to write. Two nodes on the same build can honestly answer differently.
