...
That sentence needs a second half, because for a while it was only half true. The server's deadline is measured from the moment a frame **arrives**, so it can say nothing about time a frame spent before it — and both client libraries underneath this one park an outgoing frame when the link is down and flush it on reconnect. socket.io buffers the emit; mqtt.js stores the packet and replays it verbatim, message expiry included, so the broker's own clock only starts when it finally receives it. A command published into a dead link therefore arrived later carrying a full budget, long after its caller had been told `TransportError` or `Timeout`, and nothing anywhere refused it. So **a frame is never handed to a transport that cannot send it now**: the call fails immediately with `TransportError`, which is the code that means *certainly did not run*, and it goes on meaning that.
