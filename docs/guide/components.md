...
`children` says whether the children of one branch are the same kind of thing as each other, and it decides which arrangement a viewer *opens* on. `alike` — a rack whose ports under a hub carry the same five fields — is the case where reading a column down the page is the job, so the console draws the branches as scope and their children as an aligned table. `assorted`, or absent, is a folder holding both documents and other folders, or an address space holding objects and variables: a table there is columns of blanks, so the console draws the hierarchy with the leaves in place.

It is a claim about *siblings*, not about leaves. The cabinets under the root of that rack are alike, and so are the hubs under a cabinet — `alike` does not promise that a branch's children are the end of the tree.

This is a fact about the data rather than a layout, which is the line `defaultColumns` is written to hold. It says nothing about panes, widths or order. And it chooses which arrangement opens, **never which is available**: the console offers both and remembers which the reader picked, because that is a preference and preferences belong to whoever is looking. A declaration that could not be overridden would make a wrong one cost a screen instead of a click.

A branch answer may also carry `defaultChild`: which of these children a viewer should open when the branch is opened, if any. A folder of documentation whose first business is its `README` is the case it exists for, and that is a convention of *that domain* rather than of consoles — a viewer that knew the word would be carrying somebody else's filing rule, and would apply it to a rack of serial ports the first time one had a port called readme. It is advice and bounded advice: an id the branch did not answer with is ignored, and a viewer with something already open leaves it open, because arriving somewhere and having what you were reading replaced is worse than one more click.
