...
A branch answer may also carry `grouping`, one flag per row, saying which rows are places to look inside and which are things to list. **That is not the same question as `hasChildren`**, and the difference is why it exists: an OPC UA Variable with `EngineeringUnits` hanging off it has children and is still a measurement somebody wants in a row, while an empty folder has none and is still a folder. Reading the first as an answer to the second puts the measurement in the scope tree and takes it out of the table, which is backwards — its properties are what a viewer shows *about* it, not somewhere to navigate to. Absent falls back to `hasChildren`, which is what every tree meant before this existed and is right often enough to be the default.
