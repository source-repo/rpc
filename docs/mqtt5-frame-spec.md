...
The transport-independent frame this section called for now exists, as `RPC/Frame.ts`. It is the flat form of the protocol — kind, correlation, path, method, event, and the fields a receiver acts on — and `Transports/Mqtt5Frame.ts` holds only what MQTT calls each of them. socket.io maps to the same frame, as [its own layout](socketio-frame-spec.md), so the two wire formats now differ in their framing and share their vocabulary.
