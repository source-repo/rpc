...
**Actions are drawn on the ports and not on the cabinets.** Neither declares `appliesTo`, and absent means *leaves* — which is what `resetPort` and `closePort` are about. A tree's rows are not all the same kind of thing, unlike a list's, so an action needs to say which kind it applies to or a console ends up offering a button that throws. The default is the safe half: on a flat list it shows every row, because every row of a list is a leaf.
