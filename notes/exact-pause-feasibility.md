...
And the supervisor is wired to it: `RpcPauseSupervisor` takes a barrier or a gate, exactly one, so `exactPause` can be advertised from the mechanism rather than asserted. The limits above all held. One instance per worker is now enforced by the shape rather than recommended, and a handler that reaches no gate still cannot be paused inside — it parks at the boundary before the call instead, which is a safe-boundary pause arrived at from the other direction.
