...
*The record editor.* The write contract is complete and unused. The argument form built for row actions is most of the machinery. One thing has to be settled first and is not a detail: a viewer holding a read resource has no way to find the write service that belongs to it - `document` and `relational` put writes on a separate service - so discovery has to be designed rather than assumed.
