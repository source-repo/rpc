...
**Five - one declaration.** A resource said what it *read* in `RpcDataResource.verbs` and what it *wrote* somewhere else entirely: a sibling `<namespace>.write` instance answering `writable()`. So "what can I do with this?" was two questions, of two shapes, over two round trips, joined by every caller that asked.

The duplication was the visible half. The lossy join was the half worth fixing: a write surface names a resource by a **single string** where `$data` addresses it by a **path**, so the two agreed only for resources exactly one segment deep. Anything nested - a resource under `props` or `state`, a resource two segments down - was not refused as unwritable, it was *unsayable*. `writableFor` documented this in a comment and shrugged: *"a deeper path simply matches nothing"*.

So `verbs` widened to `RpcResourceVerb = RpcDataMethod | RpcWriteVerb` and gained `columns`, and `describe()` folds in what the sibling write surface resolved. The two wire surfaces stay apart and should - `$data` is repeatable by construction, `$write` carries a precondition and answers `ok | missing | conflict` - but the *declaration* had no reason to be two. Live, `shop.customers` now reports `getList, getMany, getManyReference, update` with `columns: [name, city]`, and `shop.orders`, which no rule permits, reports its read verbs and nothing else: the write default said once, by the list being short.

Three things it decides. The write half is **resolved, not restated** - `writable()` is still the authority and this carries its answer, so a column the store does not have is absent rather than advertised and then refused. A write surface that **will not answer costs the read half nothing**, because a peer that stopped being describable when its database went down is a peer nobody can look at in the moment they most want to. And the console **stopped making the second call**: verified over the wire, three edit buttons on `customers` and zero `writable()` frames.
