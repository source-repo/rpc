...
The fifth finding arrived immediately, and it is the one that stops the feature being usable: **`contains` is case-sensitive, deliberately.** The conformance fixture keeps `borg` and `Borg AB` distinct on purpose and the SQL flavours go to some trouble to force binary collation across three engines, because a *filter* that folded case would silently change what a query means. A *search box* has the opposite requirement: somebody typing `acme` means `Acme Ltd`. So the filter language needs a way to ask for a folded comparison - a flag on the clause rather than a second set of operators - and it needs it in the in-memory matcher, in three SQL flavours, in Mongo, and in a conformance question. Until then this finds things by their name and their case.
