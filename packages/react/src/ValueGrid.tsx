...
import { RowReference } from './references.js'
import { ObjectPanel, type ObjectAccess, type Ref, type Where } from './ObjectPanel.js'
import { useDebounced, useWaitedSeconds } from './timing.js'
                ...
                {branchQuestion ? (
                    <>
                        ...
                        {preview && opensRows && opened !== undefined && (
                            <RecordPanel cache={cache} question={rowQuestion(tree.path, opened)} id={opened} period={period} columns={columns} detail={tree.presentation?.detail} sections={tree.presentation?.sections} onClose={() => setOpened(undefined)} />
                        )}
                        </div>
                    </>
                ) : (
