...
- **[Four transports, one model.](https://source-repo.github.io/rpc/guide/connecting)** socket.io for browsers and anything that dials out; MQTT 5 for the plant, with a [documented wire format](https://source-repo.github.io/rpc/mqtt5-frame-spec) a plain MQTT.js peer can speak; SignalR for reaching a .NET host. Two wire formats carry one [neutral frame](https://source-repo.github.io/rpc/flat-frame-spec), so a call means the same thing on every one of them and only the spelling differs. And a `MessagePort` for a peer on a worker thread of the same process: the same flat frame, posted rather than encoded, so there is nothing to parse and still a real boundary — a worker can be a peer with a name, a presence and a route, calling out as well as answering. Any peer can be a bus, and a browser tab can host a server.
